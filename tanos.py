	...
	while True:											#블루투스 연결중 실행하는 반복문
		...
		#받은 데이터를 utf-8로 디코드하여 가져오고, 해당 값이 판별값인 '/'인지 확인하여 유효값인지 판별한다.
			...
			with db.cursor() as cursor:		#데이터베이스의 커서를 가져옴
				...
				sql2="""SELECT value1, value2 from emg_value order by num desc limit 1"""
				...
				for row in rows:						#다수의 데이터 중
					print(row)							#첫번째 데이터(하나밖에 없다.)-괄호로 쌓여있다.
					print(row['value1'],row['value2']))	#태그를 사용하여 원하는 정보 분리-괄호 제거됨.
				
				if(int(row['value1'])>600):					#팔을 펼때
			...
			time.sleep(10)			#소켓이 닫기도록 지연시간을 둔다.
			break					#비정상 값이므로 반복문을 종료하고 블루투스 시작부분으로 되돌아감.
				
		print ("insert : "+sqlData)	#삽입한 데이터 열람 목적.
